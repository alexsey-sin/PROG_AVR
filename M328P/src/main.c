...
void init(void){
	...
	// LED L на плате
	DDR_LED_L |= (1 << B_LED_L);
	PORT_LED_L &= ~(1 << B_LED_L);
	
	PORT_I2C |= ((1 << B_SCL)|(1<<B_SCA));		// Включим подтяжку на ноги, вдруг юзер на резисторы пожмотился
	DDR_I2C &= ~((1 << B_SCL)|(1<<B_SCA));
...
// ISR(TWI_vect){ // Прерывание TWI Тут наше все.
	// switch(TWSR & 0xF8){ // Отсекаем биты прескалера
		// case 0x00:{								// Bus Fail (автобус сломался)
			// i2c_STATUS |= i2c_ERR_BF;
			// TWCR = 	0<<TWSTA| // Бит условия СТАРТ
					// 1<<TWSTO| // Бит условия СТОП
					// 1<<TWINT| // Флаг прерывания TWI: 1 = сбрасываем флаг
					// 0<<TWEA|  // Бит разрешения подтверждения: 0 = временно отключамся от шины
					// 1<<TWEN|  // Бит разрешения работы TWI
					// 1<<TWIE;  // Разрешение прерывания TWI
			// break;
		// }
		// case 0x08:{	// Старт был, а затем мы:
			...
			// if((i2c_STATUS & i2c_type_msk)== i2c_sarp)			// В зависимости от режима
		...
		// // case 0x10:{	// Повторный старт был, а затем мы
			// // // if( (i2c_STATUS & i2c_type_msk) == i2c_sawsarp)		// В зависимости от режима
				// // // {
				// // // i2c_SlaveAddress |= 0x01;					// Шлем Addr+R
				// // // }
			// // // else
		...
		// case 0x18:{	// Был послан SLA+W получили ACK, а затем:
			// if((i2c_STATUS & i2c_type_msk) == i2c_sawp){		// В зависимости от режима
			...
			// // if( (i2c_STATUS & i2c_type_msk) == i2c_sawsarp)
				// // {
		...
		// case 0x28:{ 	// Байт данных послали, получили ACK!  (если sawp - это был байт данных. если sawsarp - байт адреса страницы)
			...
			// if((i2c_STATUS & i2c_type_msk) == i2c_sawp){	// В зависимости от режима
				// if(i2c_index == i2c_ByteCount){				// Если был байт данных последний
					...
					// TWCR = 	0<<TWSTA|
							...
							// 1<<TWEN|
							// 1<<TWIE;						// Шлем Stop
					// i2c_STATUS &= ~i2c_type_msk;				// Сброс режима
			...
			// // if( (i2c_STATUS & i2c_type_msk) == i2c_sawsarp)		// В другом режиме мы
				// // {
		...
		// case 0x30:{	//Байт ушел, но получили NACK причин две. 1я передача оборвана слейвом и так надо. 2я слейв сглючил.
			// i2c_STATUS |= i2c_ERR_NK;						// Запишем статус ошибки. Хотя это не факт, что ошибка. 
		...
		// case 0x38:{	//  Коллизия на шине. Нашелся кто то поглавней
			// i2c_STATUS |= i2c_ERR_LP;						// Ставим ошибку потери приоритета
		...
		// case 0x40:{ // Послали SLA+R получили АСК. А теперь будем получать байты
			...
			// }else{										// Что даст понять слейву, что мол хватит гнать. И он отпустит шину
				// TWCR = 	0<<TWSTA|
						// 0<<TWSTO|
						// 1<<TWINT|
						// 1<<TWEA|
...
// void LCD_send_poluByte(uint8_t bt){
	...
	// i2c_Buffer[0] = ((bt & 0x00001111) << 4) | (1 << LCD_E);
	// i2c_Buffer[1] = ((bt & 0x00001111) << 4);
	
	// i2c_ByteCount = 2;
	// i2c_STATUS |= i2c_sawp;
	// TWCR = 	1 << TWSTA| // Бит условия СТАРТ
			// 0 << TWSTO| // Бит условия СТОП
			// 1 << TWINT| // Флаг прерывания TWI: 1 = сбрасываем флаг
			// 0 << TWEA|  // Бит разрешения подтверждения: 0 = временно отключамся от шины
			// 1 << TWEN|  // Бит разрешения работы TWI
...
// void LCD_send(uint8_t rs, uint8_t rw, uint8_t ld, uint8_t bt){
	...
	// i2c_Buffer[0] = (bt & 0x11110000) | (rs << LCD_RS) | (rw << LCD_RW) | (1 << LCD_E) | (ld << LCD_LED);
	// i2c_Buffer[1] = (bt & 0x11110000) | (rs << LCD_RS) | (rw << LCD_RW) | (0 << LCD_E) | (ld << LCD_LED);
	// i2c_Buffer[2] = ((bt & 0x00001111) << 4)| (rs << LCD_RS) | (rw << LCD_RW) | (1 << LCD_E) | (ld << LCD_LED);
	// i2c_Buffer[3] = ((bt & 0x00001111) << 4)| (rs << LCD_RS) | (rw << LCD_RW) | (0 << LCD_E) | (ld << LCD_LED);
