...
// внешнее прерывание тут не причем - используйте вход захвата ICP1.

// заведите сигнал с датчика на вход ICP1. запустите таймер 1.
// в прерывании переполнения увеличивайте старшую половину счетчика, в прерывании T1 capture в ICR1 получите младшую половину.
// суммарно получится 32-битное значение счетчика.

// обороты можно посчитать по формуле F_CPU * 60 / cnt.
...
// void rpm_setup() {
// TCCR1A = 0;
// TCCR1B = (1<<ICNC1)|(1<<CS10); //включить Timer1 capture, и подать сист.частоту на таймер
// TIMSK |= (1<<TICIE1)|(1<<TOIE1); //включить прерывания от ICP и переполнения таймера 1
// }

// //прерывание захвата ICP1
// ISR(TIMER1_CAPT_vect) {
// //значение счетчика за 1 оборот
// uint32_t cnt = ((uint32_t)rpm_high_cnt << 16) | ICR1;
// //обнулить мл. и ст. счетчики таймера 1
// TCNT1 = rpm_high_cnt = 0;
// //пересчитать счетчик в обороты
// rpm = (F_CPU * 60UL) / rpm_cnt;
// }

// //прерыавние переполнения Т1
// ISR(TIMER1_OVF_vect) {
// //увеличить старшую половину счетчика
// if (rpm_high_cnt < 1000) {
// rpm_high_cnt++;
// }
// }
