...
void Work(void){	//анализ и работа
	...
	while(1){
		...
		if(dbDS_ROOM >= SETVAL_MAX_TEMP_ROOM){
			NEW_BOILER_STATUS = BOILER_STATUS_STOP_ROOM;	//ОСТАНОВЛЕН по перегреву комнаты
			break;
		}
		if(dbDS_SERVE >= SETVAL_MAX_TEMP_SERVE){
			NEW_BOILER_STATUS = BOILER_STATUS_STOP_COOLANT;	//ОСТАНОВЛЕН по перегреву теплоносителя
			break;
		}
		...
		if(BOILER_STATUS == BOILER_STATUS_STOP_ROOM && dbDS_ROOM < (SETVAL_MAX_TEMP_ROOM - DEF_SETVAL_MAX_TEMP_ROOM_DELTA)){
			NEW_BOILER_STATUS = BOILER_STATUS_AUTO;
			break;
		}
		if(BOILER_STATUS == BOILER_STATUS_STOP_COOLANT && dbDS_SERVE < (SETVAL_MAX_TEMP_SERVE - DEF_SETVAL_MAX_TEMP_SERVE_DELTA)){
			NEW_BOILER_STATUS = BOILER_STATUS_AUTO;
			break;
		}
	...
	if(NEW_BOILER_STATUS != BOILER_STATUS){	//только индикация
		...
		switch(NEW_BOILER_STATUS){
			...
			case BOILER_STATUS_STOP_ROOM:{
				DrawString("ОСТАНОВЛЕН", nx, 303);		//str, poz X, poz Y
				SetFont(font_Tahoma20);
				SetColor(VGA_YELLOW);
				DrawString("Перегрев комнаты", 5, 373);		//str, poz X, poz Y
			} break;
			case BOILER_STATUS_STOP_COOLANT:{
				DrawString("ОСТАНОВЛЕН", nx, 303);		//str, poz X, poz Y
				SetFont(font_Tahoma20);
				SetColor(VGA_YELLOW);
				DrawString("Перегрев теплоносителя", 5, 341);		//str, poz X, poz Y
			} break;
	...
	if(BOILER_STATUS == BOILER_STATUS_ERROR && ERROR_ALARM_TRIGGER == ERROR_ALARM_TRIGGER_ON){	//ОШИБКА ПО ДАТЧИКАМ
		uint8_t c = 5;
		while(c--){
			SetBit(PORT_BEEP, B_BEEP);
			_delay_ms(500);
		...
		ERROR_ALARM_TRIGGER = ERROR_ALARM_TRIGGER_OFF;
	}
